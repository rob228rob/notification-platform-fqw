    ...
    model {
       ...
       // }
        ...
        platform = softwareSystem "Платформа мультиканальной доставки уведомлений" "Отказоустойчивая распределённая платформа гарантированной доставки уведомлени" {
            ...
            orchestrator = container "Notification Orchestrator" "Основная бизнес-логика: валидация команд, идемпотентность, приоритизация, выбор каналов и постановка задач в очереди без работы с сырыми контактами." "Java, Spring Boot, Kubernetes"
           ...
           //  smsDispatcher = container "SMS Channel Dispatcher" "Консьюмер SMS-очередей: читает задачи, через ContactCache проверяет контакты и согласия, отправляет SMS в провайдера, публикует результаты доставки в Kafka." "Java, Spring Boot, Kubernetes"
            emailSender = container "Email Sender" "Канальный воркер e-mail: читает задачи из Kafka, отправляет уведомления провайдеру и публикует статусы доставки." "Java, Spring Boot, Kubernetes"
            ...
            deliveryLogDb = container "Delivery Log DB" "Хранение истории уведомлений и их статусов по каналам" "PostgreSQL" {
                tags "Database"
            }
       ...
       // operator -> crm "Работает с клиентами, настройкой кампаний и анализом откликов" "Web UI"
        ...
        crm -> platform "Отправляет транзакционные уведомления" "REST/JSON"
        decisionEngine -> platform "Отправляет маркетинговые уведомления" "REST/JSON или Kafka"
       ...
       // operator -> apiGateway "Просматривает историю уведомлений и конфигурацию" "HTTPS"
        ...
        crm -> apiGateway "Создаёт транзакционные/маркетинговые уведомления" "REST/JSON"
        decisionEngine -> apiGateway "Создаёт маркетинговые уведомления" "REST/JSON"

        // Админка -> платформа
        //adminUi -> apiGateway "Запрашивает историю, DLQ, конфигурацию" "REST/JSON"

        // API Gateway к внутренним сервисам
        apiGateway -> orchestrator "Передаёт команды на доставку уведомлений (без контактов)" "REST/JSON"
       // apiGateway -> scheduler "Регистрирует отложенные и периодические уведомления" "REST/JSON"
       ...
       // apiGateway -> deliveryLogWriter "Запрашивает историю доставок для отображения в админке" "REST/JSON"

        // Планирование
        // scheduler -> orchestrator "Активирует отложенные/плановые уведомления" "REST/JSON"
        ...
        // Локальная витрина контактов и согласий
        ...
        // Репликация из CRM в витрину и Redis
      ...
      //  smsDispatcher -> contactCache "Получает контактные данные и флаги согласий для SMS" "REST/JSON"
        emailSender -> contactCache "Получает контактные данные и флаги согласий" "REST/JSON"
      ...
      //  emailSender -> kafka "Публикует результаты попыток доставки выбранного канала связи" "Kafka producers (delivery-results)"
       ...
       // smsProvider -> callbackHandlers "Присылает статусы доставки и входящие сообщения (STOP)" "HTTPS callbacks"
        ...
        callbackHandlers -> crm "Фиксирует изменения согласий в CRM" "REST/JSON"
        ...
        deliveryLogWriter -> crm "Передаёт агрегированные статусы и отчёты по кампании" "REST/JSON"
