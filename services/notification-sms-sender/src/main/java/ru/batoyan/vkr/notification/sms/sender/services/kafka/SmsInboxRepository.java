...
public class SmsInboxRepository {
    ...
    public boolean storeIncomingDispatchEvent(Map<String, Object> message) {
        return storeIncomingEvent(message, AggregateType.SMS_DISPATCH, EventType.SMS_DISPATCH_REQUESTED, "dispatch_id");
    }
    ...
    private boolean storeIncomingEvent(Map<String, Object> message, AggregateType expectedAggregateType,
                                       EventType expectedEventType, String messageIdField) {
        ...
        if (!expectedAggregateType.matches(aggregateType) || !expectedEventType.matches(eventType)) {
            log.debug("Skip kafka event aggregateType={}, eventType={}, aggregateId={}", aggregateType, eventType, aggregateId);
            return false;
        }
        ...
        try {
            jdbc.update("""
                    insert into nf_sms.consumer_inbox_message(
                      message_id, aggregate_type, aggregate_id, event_type,
                      event_id, payload, headers, processing_status, received_at
                    ) values (
                      :message_id, :aggregate_type, :aggregate_id, :event_type,
                      :event_id, cast(:payload as jsonb), cast(:headers as jsonb), :processing_status, :received_at
                    )
    ...
    public static String asString(Object value) { return value == null ? "" : String.valueOf(value); }
