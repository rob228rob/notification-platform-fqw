package ru.batoyan.vkr.sms.kafka.consumer;
...
import ru.batoyan.vkr.sms.kafka.SmsInboxRepository;
...
public class NotificationEventConsumer {
    ...
    @KafkaListener(topics = "${outbox.relay.topics.events}", groupId = "${spring.kafka.consumer.group-id}")
    public void onMessage(@Payload String rawMessage, @Headers Map<String, Object> headers) {
        var message = readMessage(rawMessage);
        var stored = inboxRepository.storeIncomingNotificationEvent(message);
        log.info("Notification event stored={}, aggregateId={}, headers={}", stored, message.get("aggregateId"), headers);
    }
